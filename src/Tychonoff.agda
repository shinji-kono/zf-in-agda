...
open import maximum-filter O HODAxiom ho< AC
...
FIP→UFLP {P} TP fip F UF = record { limit = FIP.limit fip (λ lt → CF⊆CS (eq→ *iso lt )) ufl01
     ...
     uflPQ F UF = record { limit = & < * ( UFLP.limit uflp ) , * ( UFLP.limit uflq ) >  ; P∋limit = Pf ; is-limit = isL } where
         ...
         isL {v} nei = filter1 F (λ z xz → Neighbor.v⊆P nei (eq→ *iso xz))
             ...
             F∋base {b} (gi (case1 px)) bl  = subst (λ k → odef (filter F) k) (sym (BaseP.prod px)) f∋b where
                 ...
                 isl00 = eq→ *iso ( subst (λ k → odef k lim ) (cong (*) (BaseP.prod px))  bl)
             ...
             F∋base {b} (gi (case2 qx)) bl  = subst (λ k → odef (filter F) k) (sym (BaseQ.prod qx)) f∋b where
                 ...
                 isl00 = eq→ *iso ( subst (λ k → odef k lim ) (cong (*) (BaseQ.prod qx)) bl)
